...
if len(sys.argv)>1:
    if len(sys.argv)< 7:
        ...
        print "example : python step05_prepare_augmented_data.py mfcc 512 160 True mv 500"
...
FILENAME = 'data/train_speed_vol.txt'
...
#     write_tfrecords(feat,utt_label,utt_shape,TFRECORDS_NAME)
    ...
    TFRECORDS_NAME = 'data/tfrecords/'+'mgb3_aug_'+tffilename+'_train_shuffle.'+str(i)+'.tfrecords'
    write_tfrecords(feat,utt_label,utt_shape,TFRECORDS_NAME)
    
    
FILENAME = 'data/dev_speed_vol.txt'
feat, utt_label, utt_shape, tffilename = feat_extract(FILENAME,FEAT_TYPE,N_FFT,HOP,VAD,CMVN,EXCLUDE_SHORT)
print tffilename
...
#     write_tfrecords(feat,utt_label,utt_shape,TFRECORDS_NAME)
    feat, utt_label, utt_shape = do_shuffle(feat,utt_label,utt_shape)
    TFRECORDS_NAME = 'data/tfrecords/'+'mgb3_aug_'+tffilename+'_dev_shuffle.'+str(i)+'.tfrecords'
    write_tfrecords(feat,utt_label,utt_shape,TFRECORDS_NAME)
