...
class nn:
    ...
    def net(self,x, shapes_batch,softmax_num,is_training, input_dim, is_batchnorm):   
        ...
        is_exclude_short = True
        if is_exclude_short==True and is_training == True:
            #randomly select start of sequences
            sequence_limit = tf.reduce_min(shape_list)-200
#            sequence_limit = tf.cond(sequence_limit<=200, lambda: sequence_limit, lambda: tf.subtract(sequence_limit,200))
	    ...
	    aug_list = tf.constant([200, 300, 400,500,600,700,800,900,1000, 100000000,100000000,100000000], dtype=tf.float32)
